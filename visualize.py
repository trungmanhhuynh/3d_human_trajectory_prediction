...
def plot_trajectory_pts_on_images(batch, batch_id, results, used_ped_list, args ,validation = False, test = False):
	...
	for t in range(0,args.observe_length + args.predict_length):
		...
		for i, pedId in enumerate(selectedPeds): 
			...
			if( t < args.observe_length):
				...
				idxInPredict = np.where(used_ped_list == pedId)[0]
